...
REM - This option allows you to configure the build targets for UnattendGen
REM   Refer to Microsoft documentation for support of a specific target. Separate targets with
REM   spaces
set UNATTENDGEN_BUILD_TARGETS=win-x86 win-x64 win-arm64 osx-x64 osx-arm64 linux-x64 linux-arm64
...
for %%a in (%UNATTENDGEN_BUILD_TARGETS%) do (
...
for %%a in (%UNATTENDGEN_BUILD_TARGETS%) do (
	...
	if "%%a" == "osx-x64" (
		if !UNATTENDGEN_ADD_OSX_X64_COMPAT! EQU 0 (
			...
			dotnet publish -r:%%a --framework net10.0
			rd "bin\release\net10.0\%%a\publish" /s /q
		)
	) else (
		dotnet publish -r:%%a --framework net10.0
		rd "bin\release\net10.0\%%a\publish" /s /q
